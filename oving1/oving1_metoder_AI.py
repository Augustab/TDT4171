...
class BayesianNetwork:
    ...
    def sorted_nodes(self):
        ...
        # MERK: Jeg tar en deepcopy av hele det bayesianske nettverket her for å ikke slette kantene i det originale
        # nettverket som jeg senere får brukt for!!
        ...
        # koden under sørger bare for at det kastes en feilmelding dersom ikke
...
class InferenceByEnumeration:
    ...
    def _enumeration_ask(self, X, evidence):
        ...
        Q = np.zeros((var_x.no_states, 1)) ##viktig å bruke np.array siden det er dette utgitt kode antar.
        vars_topo = []
        for name in self.topo_order:
            vars_topo.append(self.bayesian_network.variables[name])
            # her legger jeg nodene i nettverket inn i en liste i riktig rekkefølge!
    ...
    def _enumerate_all(self, vars_topo, evidence):
        ...
        # her er ok å bruke deepcopy da det ikke er objekter men strings og tall
        evidence_copy = copy.deepcopy(evidence)
        y = vars_topo_copy.pop(0)
        evidence_shortened = {}
        # for-løkken under brukes for å lage et sett med evidence som kun inneholder foreldrene til y
